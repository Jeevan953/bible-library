...
# These WMB source markers contain only footnotes.
EXPECTED_EMPTY = {
    ("LUK", 17, 36),
    ("ACT", 8, 37),
    ("ACT", 15, 34),
...
# These canonical positions remain absent after
# normalizing the WMB Romans versification.
EXPECTED_MISSING = {
    ("LUK", 17, 36),
    ("ACT", 8, 37),
    ("ACT", 15, 34),
...
def normalize_versification(source):
    ...
    for source_key, target_key in moves:
        ...
        if verse_text is None:
            raise CommandError(
                "Missing WMB source verse required for "
                f"normalization: {source_key}"
            )

        if target_key in normalized:
            raise CommandError(
                "WMB normalization target already "
                f"contains text: {target_key}"
            )

        normalized[target_key] = verse_text

    return normalized


class Command(BaseCommand):
    help = "Import the World Messianic Bible from USFM files."
    ...
    def handle(self, *args, **options):
        ...
        pdf_path = folder.parent / "wmb.pdf"
        ...
        if len(source) != 31098:
            raise CommandError(
                "Expected 31098 source texts, found "
                f"{len(source)}"
            )

        validated_source = normalize_versification(source)

        if len(validated_source) != 31098:
            raise CommandError(
                "Expected 31098 verse texts, found "
        ...
        unexpected_empty = actual_empty - EXPECTED_MISSING
        unexpectedly_present = EXPECTED_MISSING - actual_empty
        ...
        with transaction.atomic():
            ...
            version.name = "World Messianic Bible"
            version.language = "English"
            version.year = 2024
            version.description = (
                "World Messianic Bible, U.S.A. spelling "
                "edition. Public Domain; source: "
                "eBible.org. The name “World Messianic "
                "Bible” is a trademark."
            )
            version.pdf_filename = "wmb.pdf"
            version.save()
        ...
        self.stdout.write(
            f"Empty source markers: {len(EXPECTED_EMPTY)}"
        )
        self.stdout.write(
