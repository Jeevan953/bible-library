...
FILE_PATTERN = re.compile(
    r"^\d+-(?P<code>[A-Z0-9]+)eng-glw\.usfm$",
    re.IGNORECASE,
)
...
# All canonical GLW positions contain text after normalization.
...
def normalize_versification(source):
    ...
    if not verse_fourteen:
        raise CommandError(
            "Expected source text at 3 John 1:14"
        )

    if not final_greeting:
        raise CommandError(
            "Expected source text at 3 John 1:15"
        )

    normalized[("3JN", 1, 14)] = (
        f"{verse_fourteen} {final_greeting}"
    )
...
class Command(BaseCommand):
    help = "Import God’s Living Word Translation from USFM."
    ...
    def handle(self, *args, **options):
        ...
        pdf_path = folder.parent / "glw.pdf"

        if not pdf_path.is_file():
            raise CommandError(f"PDF not found: {pdf_path}")

        canonical_codes = set(GLW_CODES)
        paths = {}
        ...
        missing_files = [
            code for code in GLW_CODES if code not in paths
        ]
        ...
        if len(source_chapters) != 28:
            raise CommandError(
                "Expected 28 chapters, found "
                f"{len(source_chapters)}"
            )

        if len(source) != 1012:
            raise CommandError(
                "Expected 1012 source texts, found "
                f"{len(source)}"
            )

        normalized = normalize_versification(source)

        if len(normalized) != 1011:
            raise CommandError(
                "Expected 1011 normalized texts, found "
                f"{len(normalized)}"
            )
        ...
        with transaction.atomic():
            version, created = BibleVersion.objects.get_or_create(
                abbreviation="GLW",
                defaults={
                    "name": "God’s Living Word Translation",
                    "language": "English",
                    "year": 1996,
                },
            )

            version.name = "God’s Living Word Translation"
            version.language = "English"
            version.year = 1996
            version.description = (
                "God’s Living Word Translation, "
                "comprising the Gospel of John and "
                "1–3 John, translated by Michael "
                "Paul Johnson. Copyright © 1996 "
                "Michael Paul Johnson; source: "
                "eBible.org; licensed under "
                "CC BY-ND 4.0."
            )
            version.pdf_filename = "glw.pdf"
            version.save()
