...
class Command(BaseCommand):
    help = (
        "Import the public-domain 1835 Living Oracles New "
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "file_path",
            help="Path to the Living Oracles OSIS XML file",
        )
    ...
    def handle(self, *args, **options):
        ...
        if missing:
            raise CommandError(
                "Canonical positions without LONT text: "
                f"{missing[:20]}"
            )
        ...
        with transaction.atomic():
            version, version_created = (
                BibleVersion.objects.update_or_create(
                    abbreviation="LONT",
                    defaults={
                        "name": "Living Oracles New Testament",
                        "language": "English",
                        "year": 1835,
                        "pdf_filename": "",
                    },
        ...
        self.stdout.write(
            self.style.SUCCESS(
                ...
                "Canonical NT positions without LONT text: "
                f"{len(missing)}\n"
                "LONT positions without a canonical row: 0"
            )
        )
