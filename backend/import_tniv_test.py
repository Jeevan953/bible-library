...
def get_or_create_book(book_name):
    ...
    try:
        ...
        print(f"  📚 Created book: {book_name} (position: {position})")
        return book

def import_tniv_test(xml_file, version_abbr, books_to_import=None):
    """Import TNIV from XML file for specific books"""
    ...
    # Find all Bible books
    for book_elem in root.findall('.//BIBLEBOOK'):
        book_name = book_elem.get('bname')
        
        # Check if we should import this book
        if books_to_import and book_name not in books_to_import:
            continue
        ...
        print(f"  ✅ {book_name}: {new_verses} verses so far")
...
if __name__ == "__main__":
    # Test with just Genesis and Exodus
    print("📖 TEST IMPORT: Genesis and Exodus only")
    print("=" * 50)
    import_tniv_test('data/TNIV.xml', 'TNIV', ['Genesis', 'Exodus'])
